...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (savedUser && savedPassword) {
    ...
    if (userNameSpan) userNameSpan.textContent = savedUser;
    console.log('Credenciales cargadas desde localStorage:', { usuario: savedUser, contrasena: savedPassword });
  }
...
// Función para limpiar localStorage (opcional, para testing)
function limpiarCredenciales() {
  localStorage.removeItem('usuario');
  localStorage.removeItem('contrasena');
  const userNameSpan = document.getElementById('userName');
  if (userNameSpan) userNameSpan.textContent = '';
  alert('Credenciales eliminadas del localStorage');
}